...
namespace ANS.API.Client.Core.Tests
{
    ...
    public class ClientConnectionTests
    {
        ...
        [TestMethod]
        public async Task InvokeAsync_InvalidJsonResponse_ThrowsANSClientRequestException()
        {
            TestHttpMessageHandler handler = new TestHttpMessageHandler
            {
                Content = "invalidjson"
            ...
            await Assert.ThrowsExceptionAsync<ANSClientRequestException>(() => connection.InvokeAsync<object>(request));
        }
